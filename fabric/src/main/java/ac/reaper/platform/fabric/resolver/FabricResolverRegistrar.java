package ac.reaper.platform.fabric.resolver;

import ac.reaper.api.plugin.BasicReaperPlugin;
import ac.reaper.api.plugin.ReaperPlugin;
import ac.reaper.internal.plugin.resolver.ReaperExtensionManager;
import ac.reaper.platform.fabric.utils.message.JULoggerFactory;
...
/**
 * Manages the registration of Fabric-specific resolvers with the ReaperExtensionManager.
...
public final class FabricResolverRegistrar {
     ...
     */
    public void registerAll(ReaperExtensionManager extensionManager) {
    ...
    private ReaperPlugin resolveMod(ModContainer modContainer) {
        return modContainerCache.computeIfAbsent(modContainer, container -> {
            ...
            String folderName = metadata.getId().equals("reaperac") ? metadata.getName() : metadata.getId();
