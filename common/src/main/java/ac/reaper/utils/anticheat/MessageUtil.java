package ac.reaper.utils.anticheat;
...
import ac.reaper.utils.data.webhook.discord.CompiledDiscordTemplate;
...
public class MessageUtil {
    ...
    public @Nullable String replacePlaceholders(@Nullable ReaperPlayer player, @Nullable String string, boolean removeFormatting) {
        return replacePlaceholders(player, player == null ? null : player.platformPlayer, string, removeFormatting);
    }

    @Contract("_, null -> null; _, !null -> !null")
    public @Nullable String replacePlaceholders(@Nullable ReaperPlayer player, @Nullable String string) {
    ...
    public @Nullable String replacePlaceholders(@Nullable PlatformPlayer player, @Nullable String string) {
        return replacePlaceholders(player == null ? null : ReaperAPI.INSTANCE.getPlayerDataManager().getPlayer(player.getUniqueId()), player, string, false);
    }
    ...
    private @Nullable String replacePlaceholders(@Nullable ReaperPlayer reaperPlayer, @Nullable PlatformPlayer platformPlayer, @Nullable String string, boolean removeFormatting) {
        ...
        if (!matcher.find()) {
            return ReaperAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, string);
        }

        // Get references to the maps once, outside the loop.
        final Map<String, String> staticReplacements = ReaperAPI.INSTANCE.getExternalAPI().getStaticReplacements();
        final Map<String, Function<ReaperUser, String>> variableReplacements = ReaperAPI.INSTANCE.getExternalAPI().getVariableReplacements();
        ...
        String reaperReplaced = sb.toString();

        return ReaperAPI.INSTANCE.getMessagePlaceHolderManager().replacePlaceholders(platformPlayer, reaperReplaced).replace(PLACEHOLDER_ESCAPE_CHAR, '%');
    }

    public @NotNull Component replacePlaceholders(@NotNull ReaperPlayer player, @NotNull Component component) {
    ...
    public @NotNull Component miniMessage(@NotNull String string) {
        string = string.replace("%prefix%", ReaperAPI.INSTANCE.getConfigManager().getConfig().getStringElse("prefix", "&bReaper &8»"));
    ...
    public Component getParsedComponent(Sender sender, String key, String fallbackText) {
        String message = ReaperAPI.INSTANCE.getConfigManager().getConfig().getStringElse(key, fallbackText);
