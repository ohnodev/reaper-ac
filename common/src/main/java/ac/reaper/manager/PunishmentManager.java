...
import ac.reaper.api.event.events.CommandExecuteEvent;
import ac.reaper.checks.Check;
import ac.reaper.events.packets.ProxyAlertMessenger;
...
public class PunishmentManager implements ConfigReloadable {
    ...
    public PunishmentManager(ReaperPlayer player) {
        this.player = player;
    }
    ...
    public boolean handleAlert(ReaperPlayer player, String verbose, Check check) {
        ...
        for (PunishGroup group : groups) {
            if (group.checks.contains(check)) {
                ...
                for (ParsedCommand command : group.commands) {
                    ...
                    if (ReaperAPI.INSTANCE.getAlertManager().hasVerboseListeners() && command.command.equals("[alert]")) {
                        sentDebug = true;
                        Component component = MessageUtil.miniMessage(cmd);
                        verboseListeners = ReaperAPI.INSTANCE.getAlertManager().sendVerbose(component, null);
                    }
                    ...
                    if (violationCount >= command.threshold) {
                        ...
                        if (inInterval) {
                            ...
                            ReaperAPI.INSTANCE.getEventBus().post(executeEvent);
                            if (executeEvent.isCancelled()) continue;

                            switch (command.command) {
                                case "[webhook]" -> ReaperAPI.INSTANCE.getDiscordManager().sendAlert(player, verbose, check.getDisplayName(), vl);
                                case "[log]" -> {
                                    int vls = (int) group.violations.values().stream().filter((e) -> e == check).count();
                                    String verboseWithoutGl = verbose.replaceAll(" /gl .*", "");
                                    ReaperAPI.INSTANCE.getViolationDatabaseManager().logAlert(player, verboseWithoutGl, check.getDisplayName(), vls);
                                }
                                ...
                                case "[alert]" -> {
                                    ...
                                    if (testMode) { // secret test mode
                                        ...
                                        ReaperAPI.INSTANCE.getAlertManager().sendAlert(message, verboseListeners);
                                    }
                                }
                                default -> ReaperAPI.INSTANCE.getScheduler().getGlobalRegionScheduler().run(ReaperAPI.INSTANCE.getReaperPlugin(), () ->
                                        ReaperAPI.INSTANCE.getPlatformServer().dispatchCommand(
                                                ReaperAPI.INSTANCE.getPlatformServer().getConsoleSender(),
                                                cmd
                                        )
                                );
                            }
