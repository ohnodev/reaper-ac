package ac.reaper.platform.fabric.utils.metrics;
...
public class MetricsFabric implements Metrics {
    ...
    public MetricsFabric(ReaperPlugin plugin, int serviceId) {
        ...
        metricsBase =
                new // See https://github.com/Bastian/bstats-metrics/pull/126
                        ...
                        submitDataTask -> ReaperAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(plugin, submitDataTask),
                        () -> true,
    ...
    private void appendPlatformData(JsonObjectBuilder builder) {
        ...
        builder.appendField("onlineMode", ReaperACFabricLoaderPlugin.FABRIC_SERVER.usesAuthentication() ? 0 : 1);
        builder.appendField("bukkitVersion", ReaperAPI.INSTANCE.getPlatformServer().getPlatformImplementationString());
    ...
    private void appendServiceData(JsonObjectBuilder builder) {
        builder.appendField("pluginVersion", FabricLoader.getInstance().getModContainer("reaperac").get().getMetadata().getVersion().getFriendlyString());
    }

    private int getPlayerAmount() {
        if (ReaperACFabricLoaderPlugin.FABRIC_SERVER.isRunning()) {
            return ReaperACFabricLoaderPlugin.FABRIC_SERVER.getPlayerCount();
        } else {
            return 0;
        }
    }
