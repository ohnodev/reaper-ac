package ac.reaper.manager.violationdatabase.sqlite;

import ac.reaper.ReaperAPI;
import ac.reaper.api.plugin.ReaperPlugin;
import ac.reaper.manager.violationdatabase.DatabaseConstants;
import ac.reaper.manager.violationdatabase.DatabaseDialect;
import ac.reaper.manager.violationdatabase.DatabaseUtils;
import ac.reaper.manager.violationdatabase.Violation;
import ac.reaper.manager.violationdatabase.ViolationDatabase;
...
public class SQLiteViolationDatabase implements ViolationDatabase {
    ...
    public void connect() throws SQLException {
        try (Connection connection = getConnection()) {
            ...
            // 3. Create Lookup Table for Reaper Versions (was VERSIONS_TABLE)
            connection.prepareStatement(
                    "CREATE TABLE IF NOT EXISTS " + DatabaseConstants.REAPER_VERSIONS_TABLE + "(" +
                            "id " + pkSyntax + ", " +
                            DatabaseConstants.REAPER_VERSIONS_STRING_COLUMN + " VARCHAR(255) NOT NULL UNIQUE" +
                            ")"
            ).execute();
            connection.prepareStatement(
                    "CREATE INDEX IF NOT EXISTS idx_" + DatabaseConstants.REAPER_VERSIONS_TABLE + "_string ON " + DatabaseConstants.REAPER_VERSIONS_TABLE + "(" + DatabaseConstants.REAPER_VERSIONS_STRING_COLUMN + ")"
            ).execute();
            ...
            connection.prepareStatement(
                    "CREATE TABLE IF NOT EXISTS " + DatabaseConstants.VIOLATIONS_TABLE + "(" +
                            ...
                            DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + " INTEGER NOT NULL, " + // Reaper's version
                            ...
                            "FOREIGN KEY (" + DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + ") REFERENCES " + DatabaseConstants.REAPER_VERSIONS_TABLE + "(id), " +
            ...
            connection.prepareStatement(
                    "CREATE INDEX IF NOT EXISTS idx_" + DatabaseConstants.VIOLATIONS_TABLE + "_reaper_version_id ON " + DatabaseConstants.VIOLATIONS_TABLE + "(" + DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + ")"
            ).execute();
            connection.prepareStatement(
    ...
    public synchronized void logAlert(ReaperPlayer player, String reaperVersion, String verbose, String checkName, int vls) {
        try (
                ...
                PreparedStatement insertLog = connection.prepareStatement(
                        "INSERT INTO " + DatabaseConstants.VIOLATIONS_TABLE + " (" +
                                ...
                                DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + ", " +
            ...
            String serverName = ReaperAPI.INSTANCE.getConfigManager().getConfig().getStringElse("history.server-name", "Prison");
            ...
            long reaperVersionId = DatabaseUtils.getOrCreateId(connection, dialect, DatabaseConstants.REAPER_VERSIONS_TABLE, DatabaseConstants.REAPER_VERSIONS_STRING_COLUMN, reaperVersion);
            ...
            insertLog.setLong(7, reaperVersionId);
        ...
        try (
                ...
                PreparedStatement fetchLogs = connection.prepareStatement(
                        "SELECT " +
                                ...
                                "gv." + DatabaseConstants.REAPER_VERSIONS_STRING_COLUMN + ", " + // Select Reaper's version string
                                ...
                                "JOIN " + DatabaseConstants.REAPER_VERSIONS_TABLE + " gv ON v." + DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + " = gv.id " +
