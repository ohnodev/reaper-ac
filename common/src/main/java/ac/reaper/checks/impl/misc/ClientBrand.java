...
public class ClientBrand extends Check implements PacketCheck {
    ...
    public ClientBrand(ReaperPlayer player) {
        super(player);
    }
    ...
    private void handle(String channel, byte[] data) {
        ...
        if (data.length > 64 || data.length == 0) {
            ...
            if (!ReaperAPI.INSTANCE.getConfigManager().isIgnoredClient(brand)) {
                String message = ReaperAPI.INSTANCE.getConfigManager().getConfig().getStringElse("client-brand-format", "%prefix% &f%player% joined using %brand%");
                Component component = MessageUtil.replacePlaceholders(player, MessageUtil.miniMessage(message));

                ReaperAPI.INSTANCE.getAlertManager().sendBrand(component, null);
            }
        }
        ...
        if (hasReachHacks && ReaperAPI.INSTANCE.getConfigManager().isBlockBlacklistedForgeClients()) {
            player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, ReaperAPI.INSTANCE.getConfigManager().getDisconnectBlacklistedForge())));
        }
