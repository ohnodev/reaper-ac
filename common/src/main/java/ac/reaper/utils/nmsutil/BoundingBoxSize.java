...
import ac.reaper.utils.data.packetentity.*;
...
public final class BoundingBoxSize {
    ...
    public static float getWidth(ReaperPlayer player, PacketEntity packetEntity) {
        float width = getWidthMinusBaby(player, packetEntity);
        return width * (packetEntity.isBaby ? getBabyScaleFactor(packetEntity) : 1f);
    }

    private static float getWidthMinusBaby(ReaperPlayer player, PacketEntity packetEntity) {
    ...
    public static Vector3d getRidingOffsetFromVehicle(PacketEntity entity, ReaperPlayer player) {
        ...
        if (entity instanceof PacketEntityTrackXRot xRotEntity) {
            ...
            if (EntityTypes.isTypeInstanceOf(entity.type, EntityTypes.BOAT)) {
                ...
                vec3 = yRot(ReaperMath.radians(-xRotEntity.interpYaw) - ((float) Math.PI / 2f), vec3);
                return new Vector3d(x + vec3.x, y + (double) f1, z + vec3.z);
            } else if (entity.type == EntityTypes.LLAMA) {
                float f = player.trigHandler.cos(ReaperMath.radians(xRotEntity.interpYaw));
                float f1 = player.trigHandler.sin(ReaperMath.radians(xRotEntity.interpYaw));
                ...
                float f = player.trigHandler.sin(ReaperMath.radians(xRotEntity.interpYaw));
                float f1 = player.trigHandler.cos(ReaperMath.radians(xRotEntity.interpYaw));
    ...
    public static float getHeight(ReaperPlayer player, PacketEntity packetEntity) {
    ...
    public static double getPassengerRidingOffset(ReaperPlayer player, PacketEntity packetEntity) {
    ...
    private static float getHeightMinusBaby(ReaperPlayer player, PacketEntity packetEntity) {
        final EntityType type = packetEntity.type;
