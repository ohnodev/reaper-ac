...
import ac.reaper.predictionengine.SneakingEstimator;
import ac.reaper.predictionengine.movementtick.MovementTickerPlayer;
import ac.reaper.utils.collisions.datatypes.SimpleCollisionBox;
import ac.reaper.utils.data.KnownInput;
import ac.reaper.utils.data.Pair;
import ac.reaper.utils.data.VectorData;
import ac.reaper.utils.math.ReaperMath;
import ac.reaper.utils.math.Vec2;
...
import ac.reaper.utils.nmsutil.JumpPower;
import ac.reaper.utils.nmsutil.Riptide;
...
public class PredictionEngine {
    ...
    public static Vector3dm clampMovementToHardBorder(ReaperPlayer player, Vector3dm outputVel) {
        // TODO: Reimplement
        return outputVel;
    }

    public static Vector3dm transformInputsToVector(ReaperPlayer player, Vector3dm theoreticalInput) {
    ...
    public static Vec2 modifyInput(ReaperPlayer player, Vec2 moveVector) {
    ...
    private static float distanceToUnitSquare(Vec2 input) {
        ...
        return ReaperMath.sqrt(1.0F + ReaperMath.square(additional));
    }
    ...
    private static float getItemUseSpeedMultiplier(ReaperPlayer player) {
    ...
    public void guessBestMovement(float speed, ReaperPlayer player) {
    ...
    private void doPredictions(ReaperPlayer player, List<VectorData> possibleVelocities, float speed) {
    ...
    private Pair<Vector3dm, Vector3dm> doSeekingWallCollisions(ReaperPlayer player, Vector3dm primaryPushMovement, Vector3dm originalClientVel, VectorData clientVelAfterInput) {
    ...
    private void addZeroPointThreeToPossibilities(float speed, ReaperPlayer player, List<VectorData> possibleVelocities) {
    ...
    public void addFluidPushingToStartingVectors(ReaperPlayer player, Set<VectorData> data) {
    ...
    public Set<VectorData> fetchPossibleStartTickVectors(ReaperPlayer player) {
        ...
        // Inputs are done AFTER fluid pushing, https://github.com/MWHunter/Reaper/issues/660
    ...
    private void addNonEffectiveAI(ReaperPlayer player, Set<VectorData> data) {
    ...
    private void addAttackSlowToPossibilities(ReaperPlayer player, Set<VectorData> velocities) {
    ...
    public void applyMovementThreshold(ReaperPlayer player, Set<VectorData> velocities) {
    ...
    public void addExplosionToPossibilities(ReaperPlayer player, Set<VectorData> existingVelocities) {
    ...
    public int sortVectorData(VectorData a, VectorData b, ReaperPlayer player) {
        int aScore = 0;
        int bScore = 0;
    ...
    public Vector3dm handleStartingVelocityUncertainty(ReaperPlayer player, VectorData vector, Vector3dm targetVec) {
        ...
        // https://github.com/MWHunter/Reaper/issues/398
        ...
        //
        //
        // Reaper's old solution with hard lerping entities was to just give a ton of direct offset reduction
    ...
    public void endOfTick(ReaperPlayer player, double d) {
        player.canSwimHop = canSwimHop(player);
        player.lastWasClimbing = 0;
    }

    private void loopVectors(ReaperPlayer player, Set<VectorData> possibleVectors, float speed, List<VectorData> returnVectors) {
        // Stop omni-sprint
    ...
    public boolean canSwimHop(ReaperPlayer player) {
    ...
    public Vector3dm getMovementResultFromInput(ReaperPlayer player, Vector3dm inputVector, float f, float f2) {
        float f2InRadians = ReaperMath.radians(f2);
    ...
    public Vector3dm handleOnClimbable(Vector3dm vector, ReaperPlayer player) {
        return vector;
    }

    public void doJump(ReaperPlayer player, Vector3dm vector) {
