...
import ac.reaper.checks.impl.badpackets.BadPacketsN;
...
import ac.reaper.utils.data.TeleportAcceptData;
import ac.reaper.utils.data.TeleportData;
import ac.reaper.utils.data.VectorData;
import ac.reaper.utils.data.VelocityData;
...
import ac.reaper.utils.nmsutil.ReachUtils;
...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
    ...
    public SetbackTeleportUtil(ReaperPlayer player) {
        super(player);
    }
    ...
    public void executeForceResync() {
        if (player.gamemode == GameMode.SPECTATOR || player.disableReaper)
    ...
    private boolean isExempt() {
        ...
        if (player.disableReaper) return true;
        ...
        try {
            ...
            if (player.inVehicle()) {
                ...
                if (player.compensatedEntities.serverPlayerVehicle != null) {
                    ...
                    ReaperAPI.INSTANCE.getScheduler().getEntityScheduler().execute(player.platformPlayer, ReaperAPI.INSTANCE.getReaperPlugin(), () -> {
                        if (player.platformPlayer != null) {
                            ReaperEntity vehicle = player.platformPlayer.getVehicle();
                            if (vehicle != null) {
                                vehicle.eject();
                            }
                        }
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_7_10)) {
                y += 1.62; // 1.7 teleport offset if reaper ever supports 1.7 again
            }
    ...
    public boolean insideUnloadedChunk() {
        Column column = player.compensatedWorld.getChunk(ReaperMath.floor(player.x) >> 4, ReaperMath.floor(player.z) >> 4);

        // If true, the player is in an unloaded chunk
        return !player.disableReaper && (column == null || column.transaction() >= player.lastTransactionReceived.get() ||
