...
import ac.reaper.utils.data.tags.SyncedTag;
import ac.reaper.utils.data.tags.SyncedTags;
import ac.reaper.utils.enums.FluidTag;
import ac.reaper.utils.inventory.EnchantmentHelper;
...
public class BlockBreakSpeed {
    // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/ReaperAnticheat/Reaper/issues/2117
    ...
    public static double getBlockDamage(ReaperPlayer player, WrappedBlockState block) {
        ItemStack tool = player.inventory.getHeldItem();
        return getBlockDamage(player, tool, block.getType());
    }

    public static double getBlockDamage(ReaperPlayer player, ItemStack tool, StateType block) {
        // GET destroy speed
        ...
        final boolean canHarvest = !block.isRequiresCorrectTool() || toolSpeedData.isCorrectToolForDrop
                // temporary hardcode to workaround PE bug https://github.com/retrooper/packetevents/issues/1217; see https://github.com/ReaperAnticheat/Reaper/issues/2091
    ...
    private static float getSpeedMultiplierFromToolData(ReaperPlayer player, ItemStack tool, ToolSpeedData data) {
    ...
    private static ToolSpeedData getModernToolSpeedData(ReaperPlayer player, ItemStack tool, StateType block) {
    ...
    private static ToolSpeedData getLegacyToolSpeedData(ReaperPlayer player, ItemStack tool, StateType block) {
        ItemType toolType = tool.getType();
        float speedMultiplier = 1.0f;
        boolean isCorrectToolForDrop = false;
