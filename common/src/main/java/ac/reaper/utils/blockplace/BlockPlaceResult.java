...
import ac.reaper.utils.blockstate.helper.BlockFaceHelper;
...
import ac.reaper.utils.nmsutil.Dripstone;
...
public enum BlockPlaceResult {
    ...
    TORCH_OR_HEAD((player, place) -> {
        // type doesn't matter to reaper, same hitbox.
    ...
    // As long as one of these properties is met, it is good enough for reaper!
    VINE((player, place) -> {
    ...
    SCAFFOLDING((player, place) -> {
        ...
        } // else, cancel if the scaffolding is exactly 7 away, reaper doesn't handle this edge case yet.
