package ac.reaper.manager.violationdatabase.mysql;
...
public class MySQLViolationDatabase implements ViolationDatabase {
    ...
    public void connect() throws SQLException {
        try (Connection connection = dataSource.getConnection()) {
            ...
            // --- NEW LOOKUP TABLES ---
            // 3. Create Lookup Table for Reaper Versions
            connection.prepareStatement(
            ...
            connection.prepareStatement(
                    "CREATE TABLE IF NOT EXISTS " + DatabaseConstants.VIOLATIONS_TABLE + "(" +
                            ...
                            DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + " BIGINT NOT NULL, " + // NEW
                            ...
                            "FOREIGN KEY (" + DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + ") REFERENCES " + DatabaseConstants.REAPER_VERSIONS_TABLE + "(id), " + // NEW
            ...
            connection.prepareStatement(
                    "CREATE INDEX IF NOT EXISTS idx_" + DatabaseConstants.VIOLATIONS_TABLE + "_reaper_version_id ON " + DatabaseConstants.VIOLATIONS_TABLE + "(" + DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + ");" // NEW
            ).execute();
            connection.prepareStatement(
        ...
        try (Connection connection = dataSource.getConnection();
             ...
             PreparedStatement insertAlert = connection.prepareStatement(
                     "INSERT INTO " + DatabaseConstants.VIOLATIONS_TABLE + " (" +
                             ...
                             DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + ", " + // NEW
            ...
            insertAlert.setLong(7, reaperVersionId); // NEW
        ...
        try (Connection connection = dataSource.getConnection();
             ...
             PreparedStatement fetchLogs = connection.prepareStatement(
                     "SELECT " +
                             ...
                             "gv." + DatabaseConstants.REAPER_VERSIONS_STRING_COLUMN + ", " + // NEW
                             ...
                             "JOIN " + DatabaseConstants.REAPER_VERSIONS_TABLE + " gv ON v." + DatabaseConstants.VIOLATIONS_REAPER_VERSION_ID_COLUMN + " = gv.id " + // NEW
