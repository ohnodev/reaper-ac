...
public final class AlertManagerImpl implements AlertManager, ConfigReloadable, StartableInitable {
    ...
    @Override
    public void start() {
        platformServer = ReaperAPI.INSTANCE.getPlatformServer();
     ...
     * @throws IllegalArgumentException if the user is not a ReaperPlayer.
     * @throws NullPointerException if the ReaperPlayer's platformPlayer is null.
     */
    private @NotNull PlatformPlayer requirePlatformPlayerFromUser(@NotNull ReaperUser user) {
        ...
        if (!(user instanceof ReaperPlayer reaperPlayer)) {
            // Throw a specific exception if the type is wrong
            throw new IllegalArgumentException("AlertManager action called with non-ReaperPlayer user: " + user.getName());
        }

        PlatformPlayer platformPlayer = reaperPlayer.platformPlayer;
    ...
    public boolean hasBrandsEnabled(@NotNull ReaperUser player) {
        ReaperPlayer reaperPlayer = (ReaperPlayer) player;
        ...
        if (reaperPlayer.platformPlayer == null) return false;

        return hasBrandsEnabled(reaperPlayer.platformPlayer);
    }
    ...
    public boolean hasAlertsEnabled(Player player) {
        ...
        return hasAlertsEnabled(ReaperAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleAlerts(Player player) {
        if (player == null) return;
        toggleAlerts(ReaperAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public boolean hasVerboseEnabled(Player player) {
        if (player == null) return false;
        return hasVerboseEnabled(ReaperAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }

    @Override
    public void toggleVerbose(Player player) {
        if (player == null) return;
        toggleVerbose(ReaperAPI.INSTANCE.getPlatformPlayerFactory().getFromNativePlayerType(player));
    }
