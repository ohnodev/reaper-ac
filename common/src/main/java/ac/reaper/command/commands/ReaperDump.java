...
import ac.reaper.utils.common.PropertiesUtil;
...
public class ReaperDump implements BuildableCommand {
    ...
    private void handleDump(@NotNull CommandContext<Sender> context) {
        ...
        if (link != null) {
            sender.sendMessage(MessageUtil.miniMessage(ReaperAPI.INSTANCE.getConfigManager().getConfig()
                    .getStringElse("upload-log", "%prefix% &fUploaded debug to: %url%")
                    .replace("%url%", link)));
            return;
        }
        // TODO: change this back to application/json once allowed
        ReaperLog.sendLogAsync(sender, generateDump(), string -> link = string, "text/yaml");
    }
    ...
    public static JsonObject getDumpInfo() {
        ...
        versions.addProperty("reaper", ReaperAPI.INSTANCE.getExternalAPI().getReaperVersion());
        ...
        versions.addProperty("implementation", ReaperAPI.INSTANCE.getPlatformServer().getPlatformImplementationString());
        // state of different properties
        JsonObject states = new JsonObject();
        base.add("states", states);
        if (ReaperAPI.INSTANCE.isInitialized()) states.addProperty("platform", ReaperAPI.INSTANCE.getPlatform().toString());
        if (PAPER) states.addProperty("has_paper", true);
        // include some relevant settings if not default
        JsonObject settings = new JsonObject();
        if (ReaperAPI.INSTANCE.getAlertManager().hasConsoleVerboseEnabled()) settings.addProperty("console_verbose", true);
        if (!ReaperAPI.INSTANCE.getAlertManager().hasConsoleAlertsEnabled()) settings.addProperty("console_alerts", false);
        ...
        for (PlatformPlugin plugin : ReaperAPI.INSTANCE.getPluginManager().getPlugins()) {
    ...
    private static JsonObject getBuildInfo() {
        ...
        try {
            Properties properties = PropertiesUtil.readProperties(ReaperAPI.INSTANCE.getClass(), "reaperac.properties");
