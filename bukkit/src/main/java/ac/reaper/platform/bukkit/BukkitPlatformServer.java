...
public class BukkitPlatformServer implements PlatformServer {
    ...
    public void dispatchCommand(Sender sender, String command) {
        CommandSender commandSender = ReaperACBukkitLoaderPlugin.LOADER.getBukkitSenderFactory().reverse(sender);
        Bukkit.dispatchCommand(commandSender, command);
    }

    @Override
    public Sender getConsoleSender() {
        return ReaperACBukkitLoaderPlugin.LOADER.getBukkitSenderFactory().map(Bukkit.getConsoleSender());
    }

    @Override
    public void registerOutgoingPluginChannel(String name) {
        ReaperACBukkitLoaderPlugin.LOADER.getServer().getMessenger().registerOutgoingPluginChannel(ReaperACBukkitLoaderPlugin.LOADER, name);
    }
