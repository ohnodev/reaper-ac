package ac.reaper.utils.anticheat.update;

import ac.reaper.player.ReaperPlayer;
import ac.reaper.utils.anticheat.LogUtil;
import ac.reaper.utils.collisions.AxisSelect;
...
public class BlockPlace {
    ...
    public BlockPlace(ReaperPlayer player, InteractionHand hand, Vector3i position, int faceId, BlockFace face, ItemStack itemStack, @Nullable HitData hitData, int sequence) {
        this.player = player;
        this.hand = hand;
    ...
    private List<BlockFace> getNearestLookingDirections() {
        float pitch = ReaperMath.radians(player.pitch);
        float yaw = ReaperMath.radians(-player.yaw);
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[ReaperMath.floor(player.yaw / 90.0D + 0.5D) & 3];
    }
