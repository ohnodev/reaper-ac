package ac.reaper.manager.config;

import ac.reaper.ReaperAPI;
import ac.reaper.api.common.BasicReloadable;
...
public class ConfigManagerFileImpl implements ConfigManager, BasicReloadable {
    ...
    private File getConfigFile(String path) {
        return new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), path);
    }

    @Override
    public void reload() {
        ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder().mkdirs();
        if (!initialized) {
            initialized = true;
            config.addSource(ReaperAPI.class, "config", getConfigFile("config.yml"));
            config.addSource(ReaperAPI.class, "messages", getConfigFile("messages.yml"));
            config.addSource(ReaperAPI.class, "discord", getConfigFile("discord.yml"));
            config.addSource(ReaperAPI.class, "punishments", getConfigFile("punishments.yml"));
        }
    ...
    private void upgrade() {
        File config = new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), "config.yml");
        if (config.exists()) {
            try {
    ...
    private void removeLegacyTwoPointOne(File config) throws IOException {
        ...
        Files.move(config.toPath(), new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), "config-2.1.old.yml").toPath());
    }
    ...
    private void addMissingPunishments() {
        File config = new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), "punishments.yml");
        String configString;
        if (config.exists()) {
            try {
    ...
    private void fixBadPacketsAndAdjustPingConfig(File config, String configString) {
        ...
        File punishConfig = new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), "punishments.yml");
    ...
    private void newOffsetNewDiscordConf(File config, String configString) throws IOException {
        ...
        File discordFile = new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), "discord.yml");
    ...
    private void addSuperDebug(File config, String configString) throws IOException {
        ...
        File messageFile = new File(ReaperAPI.INSTANCE.getReaperPlugin().getDataFolder(), "messages.yml");
    ...
    private void removeAlertsOnJoin(File config, String configString) throws IOException {
        configString = configString.replaceAll("  # Should players with reaper\\.alerts permission automatically enable alerts on join\\?\r?\n  enable-on-join: (?:true|false)\r?\n", ""); // en
    ...
    private void addPacketSpamThreshold(File config, String configString) throws IOException {
        configString += "\n# Reaper sometimes cancels illegal packets such as with timer, after X packets in a second cancelled, when should\n" +
                "# we simply kick the player? This is required as some packet limiters don't count packets cancelled by reaper.\n" +
