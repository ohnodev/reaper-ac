...
import ac.reaper.predictionengine.predictions.PredictionEngine;
...
import ac.reaper.utils.enums.BoatEntityStatus;
...
public class PredictionEngineBoat extends PredictionEngine {
    public PredictionEngineBoat(ReaperPlayer player) {
    ...
    private static BoatEntityStatus getStatus(ReaperPlayer player) {
    ...
    private static @Nullable BoatEntityStatus isUnderwater(@NotNull ReaperPlayer player) {
        SimpleCollisionBox box = player.boundingBox;
        double maxBoxY = box.maxY + 0.001D;
        int minX = ReaperMath.floor(box.minX);
        int maxX = ReaperMath.ceil(box.maxX);
        int minY = ReaperMath.floor(box.maxY);
        int maxY = ReaperMath.ceil(maxBoxY);
        int minZ = ReaperMath.floor(box.minZ);
        int maxZ = ReaperMath.ceil(box.maxZ);
    ...
    private static boolean checkInWater(ReaperPlayer reaperPlayer) {
        SimpleCollisionBox box = reaperPlayer.boundingBox;
        int minX = ReaperMath.floor(box.minX);
        int maxX = ReaperMath.ceil(box.maxX);
        int minY = ReaperMath.floor(box.minY);
        int maxY = ReaperMath.ceil(box.minY + 0.001D);
        int minZ = ReaperMath.floor(box.minZ);
        int maxZ = ReaperMath.ceil(box.maxZ);
        boolean inWater = false;
        reaperPlayer.vehicleData.waterLevel = -Double.MAX_VALUE;

        for (int x = minX; x < maxX; ++x) {
            for (int y = minY; y < maxY; ++y) {
                for (int z = minZ; z < maxZ; ++z) {
                    double level = reaperPlayer.compensatedWorld.getWaterFluidLevelAt(x, y, z);
                    if (level > 0) {
                        float f = (float) ((float) y + level);
                        reaperPlayer.vehicleData.waterLevel = Math.max(f, reaperPlayer.vehicleData.waterLevel);
    ...
    public static float getGroundFriction(ReaperPlayer player) {
    ...
    private void floatBoat(ReaperPlayer player, Vector3dm vector) {
    ...
    public float getWaterLevelAbove(ReaperPlayer player) {
    ...
    private void controlBoat(ReaperPlayer player, Vector3dm vector, boolean intermediate) {
        float f = 0.0F;
        ...
        vector.add(new Vector3dm(player.trigHandler.sin(ReaperMath.radians(-player.yaw)) * f, 0, (double) (player.trigHandler.cos(ReaperMath.radians(player.yaw)) * f)));
    }
}
