 ...
 * Someone trying to false reaper by using negative levitation effects
...
public class PointThreeEstimator {
    ...
    public PointThreeEstimator(ReaperPlayer player) {
        this.player = player;
    }
    ...
    public void handleChangeBlock(int x, int y, int z, WrappedBlockState state) {
        ...
        if (pointThreeBox.isIntersected(new SimpleCollisionBox(x, y, z))) {
            // https://github.com/MWHunter/Reaper/issues/613
    ...
    private double iterateGravity(ReaperPlayer player, double y) {
