package ac.reaper.player;

import ac.reaper.ReaperAPI;
import ac.reaper.api.AbstractCheck;
import ac.reaper.api.ReaperUser;
import ac.reaper.api.PacketWorld;
import ac.reaper.api.config.ConfigManager;
import ac.reaper.api.handler.ResyncHandler;
import ac.reaper.checks.Check;
import ac.reaper.checks.impl.aim.processor.AimProcessor;
import ac.reaper.checks.impl.misc.ClientBrand;
import ac.reaper.checks.impl.misc.TransactionOrder;
import ac.reaper.checks.impl.packetorder.PacketOrderProcessor;
import ac.reaper.events.packets.CheckManagerListener;
import ac.reaper.manager.*;
import ac.reaper.manager.player.features.FeatureManagerImpl;
import ac.reaper.manager.player.handlers.DefaultResyncHandler;
import ac.reaper.manager.player.handlers.NoOpResyncHandler;
import ac.reaper.platform.api.player.PlatformPlayer;
import ac.reaper.predictionengine.MovementCheckRunner;
import ac.reaper.predictionengine.PointThreeEstimator;
import ac.reaper.predictionengine.UncertaintyHandler;
import ac.reaper.manager.AttackCooldownHandler;
...
import ac.reaper.utils.change.PlayerBlockHistory;
import ac.reaper.utils.collisions.datatypes.SimpleCollisionBox;
import ac.reaper.utils.data.*;
import ac.reaper.utils.data.packetentity.PacketEntity;
import ac.reaper.utils.data.packetentity.PacketEntityHappyGhast;
import ac.reaper.utils.data.packetentity.PacketEntitySelf;
import ac.reaper.utils.data.tags.SyncedTags;
import ac.reaper.utils.enums.FluidTag;
import ac.reaper.utils.enums.Pose;
import ac.reaper.utils.latency.*;
import ac.reaper.utils.math.ReaperMath;
import ac.reaper.utils.math.Location;
import ac.reaper.utils.math.TrigHandler;
...
import ac.reaper.utils.nmsutil.Materials;
...
public class ReaperPlayer implements ReaperUser {
    ...
    @Getter @Setter private ResyncHandler resyncHandler = ReaperAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("disable-default-resync-handler", false) ? NoOpResyncHandler.INSTANCE : new DefaultResyncHandler(this);
    ...
    // This variable is for support with test servers that want to be able to disable reaper
    // Reaper disabler 2022 still working!
    public boolean disableReaper = false;
    ...
    public ReaperPlayer(@NotNull User user) {
        this.user = user;
    ...
    public void onPacketCancel() {
        if (spamThreshold != -1 && cancelledPackets.incrementAndGet() > spamThreshold) {
            ...
            disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, ReaperAPI.INSTANCE.getConfigManager().getDisconnectClosed())));
    ...
    public void sendTransaction(boolean async) {
        ...
        if (disableReaper && (System.nanoTime() - getPlayerClockAtLeast()) > 15e9) {
            return;
        }
    ...
    public void timedOut() {
        disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(this, ReaperAPI.INSTANCE.getConfigManager().getDisconnectTimeout())));
    }
    ...
    public void disconnect(Component reason) {
        ...
        if (platformPlayer != null) {
            ReaperAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, ReaperAPI.INSTANCE.getReaperPlugin(),
    ...
    public void pollData() {
        ...
        if (!ReaperAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            ReaperAPI.INSTANCE.getPlayerDataManager().remove(user);
        }

        if (uuid != null && this.platformPlayer == null) {
            this.platformPlayer = ReaperAPI.INSTANCE.getPlatformPlayerFactory().getFromUUID(uuid);
            updatePermissions();
        }
    }
    ...
    public void updatePermissions() {
        ...
        try {
            ReaperAPI.INSTANCE.getScheduler().getEntityScheduler().execute(platformPlayer, ReaperAPI.INSTANCE.getReaperPlugin(), () -> {
                this.noModifyPacketPermission = platformPlayer.hasPermission("reaper.nomodifypacket");
                this.noSetbackPermission = platformPlayer.hasPermission("reaper.nosetback");
    ...
    public int getTransactionPing() {
        return ReaperMath.floor(transactionPing / 1e6);
    }
    ...
    public final void reload(ConfigManager config) {
        ...
        maxTransactionTime = ReaperMath.clamp(config.getIntElse("max-transaction-time", 60), 1, 180);
    ...
    public void resyncPositions(SimpleCollisionBox box) {
        this.resyncHandler.resync(ReaperMath.floor(box.minX), ReaperMath.floor(box.minY), ReaperMath.floor(box.minZ),
                ReaperMath.ceil(box.maxX), ReaperMath.ceil(box.maxY), ReaperMath.ceil(box.maxZ));
    }

    public void addMovementThisTick(ReaperPlayer.Movement movement) {
        if (this.movementThisTick.size() >= 100) {
            ReaperPlayer.Movement movement1 = this.movementThisTick.removeFirst();
            ReaperPlayer.Movement movement2 = this.movementThisTick.removeFirst();
            ReaperPlayer.Movement movement3 = new ReaperPlayer.Movement(movement1.from(), movement2.to());
