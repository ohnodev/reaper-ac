...
import ac.reaper.utils.nmsutil.Ray;
...
public class SimpleCollisionBox implements CollisionBox {
    ...
    public Vector3d getCenter() {
        return new Vector3d(ReaperMath.lerp(0.5, this.minX, this.maxX), ReaperMath.lerp(0.5, this.minY, this.maxY), ReaperMath.lerp(0.5, this.minZ, this.maxZ));
    }
    ...
    public static Vector3i containing(double x, double y, double z) {
        return new Vector3i(ReaperMath.floor(x), ReaperMath.floor(y), ReaperMath.floor(z));
    }
    ...
    public static Iterable<Vector3i> betweenCornersInDirection(SimpleCollisionBox boundingBox, Vector3d directionVector) {
        ...
        int minX = ReaperMath.floor(min.x);
        int minY = ReaperMath.floor(min.y);
        int minZ = ReaperMath.floor(min.z);
        Vector3d max = boundingBox.max().toVector3d();
        int maxX = ReaperMath.floor(max.x);
        int maxY = ReaperMath.floor(max.y);
        int maxZ = ReaperMath.floor(max.z);
