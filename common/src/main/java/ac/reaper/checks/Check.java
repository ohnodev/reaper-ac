...
import ac.reaper.api.event.events.FlagEvent;
...
@Getter
public class Check extends ReaperProcessor implements AbstractCheck {
    protected @NotNull final ReaperPlayer player;
    ...
    public Check(final @NotNull ReaperPlayer player) {
    ...
    public final void updatePermissions() {
        ...
        exemptPermission = player.platformPlayer.hasPermission("reaper.exempt." + id);
        noSetbackPermission = player.platformPlayer.hasPermission("reaper.nosetback." + id);
        noModifyPacketPermission = player.platformPlayer.hasPermission("reaper.nomodifypacket." + id);
    }
    ...
    public final boolean flag(String verbose) {
        if (player.disableReaper || (experimental && !player.isExperimentalChecks()) || exemptPermission)
            return false; // Avoid calling event if disabled

        FlagEvent event = new FlagEvent(player, this, verbose);
        ReaperAPI.INSTANCE.getEventBus().post(event);
