...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "Unknown arg: $1"
      echo "Usage: $0 --case name --mc-version 26.1 --server-dir tests/runs/name --profile official-261|intermediary-1216 [--packetevents-repo /root/packetevents-reaper-restructure]"
      exit 1
      ;;
  esac
done
...
echo "[bootstrap:${CASE_NAME}] Building Reaper fabric artifact (local override enabled)..."
...
echo "[bootstrap:${CASE_NAME}] Installing Reaper + PacketEvents jars (${PROFILE})..."
"${SCRIPT_DIR}/install-reaper-pe-jars.sh" \
  --server-dir "${SERVER_DIR}" \
