...
import ac.reaper.utils.data.MainSupportingBlockData;
import ac.reaper.utils.data.packetentity.PacketEntity;
import ac.reaper.utils.data.packetentity.PacketEntityHorse;
import ac.reaper.utils.data.packetentity.PacketEntityNautilus;
import ac.reaper.utils.data.packetentity.PacketEntityStrider;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float f, ReaperPlayer player) {
     ...
     */
    public static StateType getOnPos(ReaperPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(ReaperPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(ReaperPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(ReaperPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(ReaperPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)
                ? new Vector3i(ReaperMath.floor(playerPos.getX()), ReaperMath.floor(playerPos.getY() - 0.5000001), ReaperMath.floor(playerPos.getZ()))
                : getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getBlockType(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(ReaperPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(ReaperMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(ReaperMath.floor(playerPos.getX()), ReaperMath.floor(playerPos.getY() - searchBelowPlayer), ReaperMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(ReaperPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(ReaperPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getBlockType(ReaperMath.floor(x), ReaperMath.floor(y - 0.2F), ReaperMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getBlockType(ReaperMath.floor(x), ReaperMath.floor(y - 1.2F), ReaperMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(ReaperPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(ReaperPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(ReaperPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) ReaperMath.lerp((float) player.compensatedEntities.self.getAttributeValue(Attributes.MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }
}
