package ac.reaper.utils.data.attribute;
...
import static ac.reaper.utils.latency.CompensatedEntities.SPRINTING_MODIFIER_UUID;
...
public final class ValuedAttribute {
     ...
     */
    public ValuedAttribute requiredVersion(ReaperPlayer player, ClientVersion requiredVersion) {
    ...
    public double with(WrapperPlayServerUpdateAttributes.Property property) {
        ...
        double newValue =
                ReaperMath.clamp((baseValue + additionSum) * (1 + multiplyBaseSum) * multiplyTotalProduct,
                        min, max);
        if (setRewriter != null) {
