...
public class ReaperVersion implements BuildableCommand {
    ...
    public static void checkForUpdatesAsync(Sender sender) {
        String current = ReaperAPI.INSTANCE.getExternalAPI().getReaperVersion();
        sender.sendMessage(Component.text()
                .append(Component.text("Reaper Version: ").color(NamedTextColor.GRAY))
        ...
        lastCheck = now;
        ReaperAPI.INSTANCE.getScheduler().getAsyncScheduler().runNow(ReaperAPI.INSTANCE.getReaperPlugin(), () -> checkForUpdates(sender));
    }
    ...
    private static void checkForUpdates(Sender sender) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(CommonReaperArguments.API_URL.value() + "updates"))
                    .GET()
                    .header("User-Agent", "ReaperAC/" + ReaperAPI.INSTANCE.getExternalAPI().getReaperVersion())
            ...
            if (statusCode < 200 || statusCode >= 300) {
                ...
                sender.sendMessage(Objects.requireNonNullElseGet(msg, () -> Component.text()
                        ...
                        .append(Component.text(" Failed to check latest ReaperAC version. Update server responded with status code: ")
            ...
            if (object.has("status")) {
                ...
                status = Status.SemVer.getVersionStatus(ReaperAPI.INSTANCE.getExternalAPI().getReaperVersion(), latest);
            }
            //
            Component msg = switch (status) {
                case AHEAD ->
                        Component.text("You are using a development version of ReaperAC").color(NamedTextColor.LIGHT_PURPLE);
                case UPDATED ->
                        Component.text("You are using the latest version of ReaperAC").color(NamedTextColor.GREEN);
                case OUTDATED -> Component.text()
                        .append(Component.text("New ReaperAC version found!").color(NamedTextColor.AQUA))
                        ...
                        .build();
                case UNKNOWN ->
                        Component.text("You are using an unknown ReaperAC version.").color(NamedTextColor.RED);
            };
            ...
            LogUtil.error("Failed to check latest ReaperAC version.", e);
        }
    }
