...
import ac.reaper.utils.data.webhook.discord.Embed;
import ac.reaper.utils.data.webhook.discord.EmbedField;
import ac.reaper.utils.data.webhook.discord.EmbedFooter;
...
public class DiscordManager implements StartableInitable, ReloadableInitable {
    ...
    private static String validatedConfigURL(String configPath, String defaultURL) {
        String url = ReaperAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-image-url", defaultURL);
    ...
    public void reload() {
        try {
            ...
            ConfigManager config = ReaperAPI.INSTANCE.getConfigManager().getConfig();
            ...
            embedFooterUrl = validatedConfigURL("embed-footer-url", "https://reaper.ac/images/reaper.png");
            embedFooterText = config.getStringElse("embed-footer-text", "v%reaper_version%");
            embedTitle = config.getStringElse("embed-title", "**Reaper Alert**");
    ...
    public void sendAlert(@NotNull ReaperPlayer player, String verbose, String checkName, int violations) {
        if (isDisabled()) {
            return;
        }

        // Per-alert overlay — avoids polluting the global static map
        Map<String, String> statics = new HashMap<>(ReaperAPI.INSTANCE.getExternalAPI().getStaticReplacements());
        statics.put("%check%", checkName);
        statics.put("%violations%", Integer.toString(violations));

        Map<String, Function<ReaperUser, String>> dynamics = ReaperAPI.INSTANCE.getExternalAPI().getVariableReplacements();
    ...
    public CompletableFuture<Boolean> sendWebhookMessage(WebhookMessage message) {
        ...
        if (!taskStarted.getAndSet(true)) {
            ...
            ReaperAPI.INSTANCE.getScheduler().getAsyncScheduler().runAtFixedRate(ReaperAPI.INSTANCE.getReaperPlugin(), DiscordManager::tick, 0, 1);
        }
