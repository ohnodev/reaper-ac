...
public class JumpPower {
    public static void jumpFromGround(@NotNull ReaperPlayer player, @NotNull Vector3dm vector) {
        ...
        if (player.isSprinting) {
            float radRotation = ReaperMath.radians(player.yaw);
    ...
    public static float getJumpPower(@NotNull ReaperPlayer player) {
        return (float) player.compensatedEntities.self.getAttributeValue(Attributes.JUMP_STRENGTH) * getPlayerJumpFactor(player);
    }

    public static float getPlayerJumpFactor(@NotNull ReaperPlayer player) {
