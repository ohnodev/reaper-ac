package ac.reaper.utils.common.arguments;

import ac.reaper.platform.api.Platform;

import static ac.reaper.utils.common.arguments.ArgumentUtils.platform;
import static ac.reaper.utils.common.arguments.ArgumentUtils.string;

public class CommonReaperArguments {

    private final static SystemArgumentFactory FACTORY = SystemArgumentFactory.Builder.of("Reaper")
            .optionModifier(builder -> builder.key("Reaper" + builder.options().getKey()))
            .supportEnv()
            .build();
    ...
    public final static SystemArgument<String> API_URL = FACTORY.create(string("APIUrl", "https://api.reaper.ac/v1/server/"));
    public final static SystemArgument<String> PASTE_URL = FACTORY.create(string("PasteUrl", "https://paste.reaper.ac/"));
    public final static SystemArgument<Platform> PLATFORM_OVERRIDE = FACTORY.create(platform("PlatformOverride"));

    /**
     * Enables "Fast Bypass" mode for chat messages sent by ReaperAC.
     * <p>
