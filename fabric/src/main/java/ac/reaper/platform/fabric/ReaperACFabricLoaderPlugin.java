...
import ac.reaper.platform.api.manager.*;
...
import ac.reaper.platform.fabric.manager.*;
import ac.reaper.platform.fabric.player.FabricPlatformPlayerFactory;
import ac.reaper.platform.fabric.resolver.FabricResolverRegistrar;
import ac.reaper.platform.fabric.scheduler.FabricPlatformScheduler;
...
public abstract class ReaperACFabricLoaderPlugin implements PlatformLoader {
    public static MinecraftServer FABRIC_SERVER;
    public static ReaperACFabricLoaderPlugin LOADER;
    ...
    protected final ReaperPlugin plugin;
    @Getter
    ...
    public ReaperACFabricLoaderPlugin(
        ...
        ReaperExtensionManager extensionManager = ReaperAPI.INSTANCE.getExtensionManager();
        resolverRegistrar.registerAll(extensionManager);
        plugin = extensionManager.getPlugin("ReaperAC");
    }
    ...
    @Override
    public ReaperPlugin getPlugin() {
        return plugin;
    }

    @Override
    public void registerAPIService() {
        ReaperAPIProvider.init(ReaperAPI.INSTANCE.getExternalAPI());
    }
    ...
    private CommandService createCommandService() {
        try {
            ...
            LogUtil.warn("IMPORTANT: Command Framework failed to load (Missing Cloud Library?). \n" +
                    "Reaper will run without commands enabled!");
