...
public class LatencyUtils {
    ...
    public LatencyUtils(ReaperPlayer player) {
        this.player = player;
    }
        ...
        synchronized (this) {
            ...
            for (Runnable runnable : tasksToRun) {
                try {
                    ...
                    if (CommonReaperArguments.KICK_ON_TRANSACTION_ERRORS.value()) {
                        player.disconnect(MessageUtil.miniMessage(MessageUtil.replacePlaceholders(player, ReaperAPI.INSTANCE.getConfigManager().getDisconnectPacketError())));
                    }
                }
            }
        }
