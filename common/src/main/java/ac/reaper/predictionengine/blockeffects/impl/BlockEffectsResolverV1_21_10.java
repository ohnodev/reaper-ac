...
import ac.reaper.predictionengine.blockeffects.BlockCollisions;
import ac.reaper.predictionengine.blockeffects.BlockEffectsResolver;
import ac.reaper.predictionengine.blockeffects.BlockStepVisitor;
...
public class BlockEffectsResolverV1_21_10 implements BlockEffectsResolver {
    ...
    public static int checkInsideBlocks(ReaperPlayer player, Vector3d from, Vector3d to, LongSet visitedBlocks, int count) {
        SimpleCollisionBox boundingBox = GetBoundingBox.getCollisionBoxForPlayer(player, to.x, to.y, to.z).expand(-1.0E-5F);
        boolean isFarEnough = from.distanceSquared(to) > ReaperMath.square(0.9999900000002526);
    ...
    public static boolean forEachBlockIntersectedBetween(Vector3d start, Vector3d end, SimpleCollisionBox boundingBox, BlockStepVisitor visitor) {
        ...
        if (direction.lengthSquared() < (double) ReaperMath.square(1.0E-5F)) {
            ...
            for (Vector3i blockPos : SimpleCollisionBox.betweenCornersInDirection(boundingBox.move(direction.multiply(-1.0)), direction)) {
                ...
                alreadyVisited.add(ReaperMath.asLong(blockPos));
            }
            ...
            if (iterationCount < 0) {
                ...
                for (Vector3i blockPos : SimpleCollisionBox.betweenCornersInDirection(boundingBox, direction)) {
                    if (alreadyVisited.add(ReaperMath.asLong(blockPos)) && !visitor.visit(blockPos, iterationCount + 1)) {
                        return false;
                    }
                }
    ...
    public static int addCollisionsAlongTravel(LongSet alreadyVisited, Vector3d direction, SimpleCollisionBox boundingBox, BlockStepVisitor visitor) {
        ...
        int currentX = ReaperMath.floor(start.x);
        int currentY = ReaperMath.floor(start.y);
        int currentZ = ReaperMath.floor(start.z);
        int stepX = ReaperMath.sign(direction.x);
        int stepY = ReaperMath.sign(direction.y);
        int stepZ = ReaperMath.sign(direction.z);
        ...
        double tDeltaX = tMaxX * (stepX > 0 ? 1.0 - ReaperMath.frac(start.x) : ReaperMath.frac(start.x));
        double tDeltaY = tMaxY * (stepY > 0 ? 1.0 - ReaperMath.frac(start.y) : ReaperMath.frac(start.y));
        double tDeltaZ = tMaxZ * (stepZ > 0 ? 1.0 - ReaperMath.frac(start.z) : ReaperMath.frac(start.z));
        ...
        while (tDeltaX <= 1.0 || tDeltaY <= 1.0 || tDeltaZ <= 1.0) {
            ...
            if (!collisionPoint.isEmpty()) {
                ...
                double clampedX = ReaperMath.clamp(collisionVec.x, currentX + 1.0E-5F, currentX + 1.0 - 1.0E-5F);
                double clampedY = ReaperMath.clamp(collisionVec.y, currentY + 1.0E-5F, currentY + 1.0 - 1.0E-5F);
                double clampedZ = ReaperMath.clamp(collisionVec.z, currentZ + 1.0E-5F, currentZ + 1.0 - 1.0E-5F);
                int endX = ReaperMath.floor(clampedX - sizeX * furthestCorner.getX());
                int endY = ReaperMath.floor(clampedY - sizeY * furthestCorner.getY());
                int endZ = ReaperMath.floor(clampedZ - sizeZ * furthestCorner.getZ());
                ...
                for (Vector3i blockPos : SimpleCollisionBox.betweenCornersInDirection(currentX, currentY, currentZ, endX, endY, endZ, direction)) {
                    if (alreadyVisited.add(ReaperMath.asLong(blockPos)) && !visitor.visit(blockPos, copyIterationCount)) {
                        return -1;
                    }
                }
            }
