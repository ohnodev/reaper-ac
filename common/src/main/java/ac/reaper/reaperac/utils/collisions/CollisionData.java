package ac.reaper.reaperac.utils.collisions;

import ac.reaper.reaperac.player.GrimPlayer;
import ac.reaper.reaperac.predictionengine.movementtick.MovementTickerStrider;
import ac.reaper.reaperac.utils.collisions.blocks.DoorHandler;
import ac.reaper.reaperac.utils.collisions.blocks.DynamicChest;
import ac.reaper.reaperac.utils.collisions.blocks.DynamicChorusPlant;
import ac.reaper.reaperac.utils.collisions.blocks.DynamicStair;
import ac.reaper.reaperac.utils.collisions.blocks.PistonBaseCollision;
import ac.reaper.reaperac.utils.collisions.blocks.PistonHeadCollision;
import ac.reaper.reaperac.utils.collisions.blocks.TrapDoorHandler;
import ac.reaper.reaperac.utils.collisions.blocks.connecting.DynamicCollisionFence;
import ac.reaper.reaperac.utils.collisions.blocks.connecting.DynamicCollisionPane;
import ac.reaper.reaperac.utils.collisions.blocks.connecting.DynamicCollisionWall;
...
import ac.reaper.reaperac.utils.collisions.datatypes.DynamicCollisionBox;
import ac.reaper.reaperac.utils.collisions.datatypes.HexCollisionBox;
import ac.reaper.reaperac.utils.collisions.datatypes.HexOffsetCollisionBox;
...
public enum CollisionData implements CollisionFactory {
    ...
    BAMBOO((player, version, block, x, y, z) -> new HexOffsetCollisionBox(block.getType(), 6.5D, 0.0D, 6.5D, 9.5D, 16.0D, 9.5D), StateTypes.BAMBOO),
    ...
    STONE_CUTTER((player, version, data, x, y, z) -> new HexCollisionBox(0.0D, 0.0D, 0.0D, 16.0D, 9.0D, 16.0D), StateTypes.STONECUTTER),
    ...
    SNOW((player, version, data, x, y, z) -> {
        ...
        if (layers == 1) return NoCollisionBox.INSTANCE;
    ...
    FRAME((player, version, data, x, y, z) -> {
        ...
        if (data.isEye()) {
            // 1.13 players have a 0.5x0.5 eye
    ...
    CARPET(new SimpleCollisionBox(0.0F, 0.0F, 0.0F, 1.0F, 0.0625F, 1.0F, false),
    ...
    FARMLAND(new HexCollisionBox(0.0D, 0.0D, 0.0D, 16.0D, 15.0D, 16.0D), StateTypes.FARMLAND),

    GRASS_PATH(new HexCollisionBox(0.0D, 0.0D, 0.0D, 16.0D, 15.0D, 16.0D),
    ...
    CONDUIT(new HexCollisionBox(5.0D, 5.0D, 5.0D, 11.0D, 11.0D, 11.0D), StateTypes.CONDUIT),
    ...
    MUD_BLOCK((player, version, data, x, y, z) -> new HexCollisionBox(0.0D, 0.0D, 0.0D, 16.0D, 14.0D, 16.0D), StateTypes.MUD),
    ...
    COPPER_GOLEM_STATUE((player, version, data, x, y, z) -> new HexCollisionBox(3, 0, 3, 13, 14, 13), BlockTags.COPPER_GOLEM_STATUES.getStates().toArray(new StateType[0])),
