package ac.reaper.platform.fabric.sender;
...
public class FabricSenderFactory extends SenderFactory<CommandSourceStack> {
    ...
    private final AbstractFabricPlatformServer platformServer = ReaperACFabricLoaderPlugin.LOADER.getPlatformServer();
    private final IFabricMessageUtil fabricMessageUtils = ReaperACFabricLoaderPlugin.LOADER.getFabricMessageUtils();
    ...
    protected void sendMessage(CommandSourceStack sender, Component message) {
        fabricMessageUtils.sendMessage(sender, ReaperACFabricLoaderPlugin.LOADER.getFabricConversionUtil().toNativeText(message), false);
    }
