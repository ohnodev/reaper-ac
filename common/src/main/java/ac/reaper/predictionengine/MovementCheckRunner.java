...
import ac.reaper.checks.impl.vehicle.VehicleC;
import ac.reaper.checks.type.PositionCheck;
import ac.reaper.manager.SetbackTeleportUtil;
import ac.reaper.player.ReaperPlayer;
import ac.reaper.predictionengine.movementtick.MovementTickerCamel;
import ac.reaper.predictionengine.movementtick.MovementTickerHappyGhast;
import ac.reaper.predictionengine.movementtick.MovementTickerHorse;
import ac.reaper.predictionengine.movementtick.MovementTickerNautilus;
import ac.reaper.predictionengine.movementtick.MovementTickerPig;
import ac.reaper.predictionengine.movementtick.MovementTickerPlayer;
import ac.reaper.predictionengine.movementtick.MovementTickerStrider;
import ac.reaper.predictionengine.predictions.PredictionEngineNormal;
import ac.reaper.predictionengine.predictions.rideable.PredictionEngineBoat;
import ac.reaper.predictionengine.predictions.rideable.PredictionEngineRideableUtils;
...
import ac.reaper.utils.data.SetBackData;
...
import ac.reaper.utils.data.packetentity.PacketEntityTrackXRot;
...
import ac.reaper.utils.nmsutil.BoundingBoxSize;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(ReaperPlayer player) {
        super(player);
    }
    ...
    private void handleTeleport(PositionUpdate update) {
        ...
        //
        // However, for some reason, this behaviour does not work correctly in Reaper
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, ReaperMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, ReaperMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, ReaperMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, ReaperMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.vehicleData.wasVehicleSwitch || player.vehicleData.lastDummy) {
            ...
            if (riding != null) {
                ...
                // their similar code may also work on reaper.
                //
        ...
        } else if (player.disableReaper || (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_8) && player.gamemode == GameMode.SPECTATOR) || player.isFlying || (player.isExemptElytra() && player.isGliding)) {
