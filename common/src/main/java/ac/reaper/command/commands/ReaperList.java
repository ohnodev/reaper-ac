...
import ac.reaper.command.CommandUtils;
...
public class ReaperList implements BuildableCommand {

    // Mainly for debugging purposes. Useful for seeing which players are exempt or not.

    @Override
    public void register(CommandManager<Sender> commandManager, CloudCommandAdapter adapter) {
        commandManager.command(commandManager.commandBuilder("reaper", "reaperac")
                .literal("list")
                .permission("reaper.list")
    ...
    private void handleListPlayers(Sender sender) {
        ...
        final Map<UUID, PlatformPlayer> onlinePlayers = ReaperAPI.INSTANCE.getPlatformPlayerFactory().getOnlinePlayers()
                .stream().collect(Collectors.toMap(ReaperIdentity::getUniqueId, Function.identity()));
        //
        ...
        //
        for (ReaperPlayer entry : ReaperAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (after) {
            ...
            boolean exempt = !ReaperAPI.INSTANCE.getPlayerDataManager().shouldCheck(entry.user);
