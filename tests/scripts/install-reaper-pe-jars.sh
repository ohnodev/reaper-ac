...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "Unknown arg: $1"
      echo "Usage: $0 [--server-dir tests/runs/fabric-26.1] [--packetevents-repo /root/packetevents-reaper-restructure] [--profile official-261|intermediary-1216] [--fabric-api-version 0.144.4+26.1]"
      exit 1
      ;;
  esac
done
...
REAPER_LIBS="${REPO_ROOT}/fabric/build/libs"
...
echo "[install] Cleaning existing Reaper/PacketEvents jars in ${MODS_DIR}"
rm -f "${MODS_DIR}"/reaperac-fabric*.jar "${MODS_DIR}"/packetevents*.jar

echo "[install] Copying Reaper jar:"
reaper_jar="$(latest_non_doc_jar "${REAPER_LIBS}" "reaperac-fabric-*.jar" || true)"
