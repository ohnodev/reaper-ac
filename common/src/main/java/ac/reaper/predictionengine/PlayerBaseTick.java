package ac.reaper.predictionengine;

import ac.reaper.player.ReaperPlayer;
import ac.reaper.utils.collisions.datatypes.SimpleCollisionBox;
import ac.reaper.utils.data.attribute.ValuedAttribute;
...
import ac.reaper.utils.latency.CompensatedEntities;
...
import ac.reaper.utils.nmsutil.CheckIfChunksLoaded;
import ac.reaper.utils.nmsutil.Collisions;
import ac.reaper.utils.nmsutil.FluidTypeFlowing;
...
public final class PlayerBaseTick {
    ...
    public static boolean canEnterPose(ReaperPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    private static SimpleCollisionBox getBoundingBoxForPose(ReaperPlayer player, Pose pose, double x, double y, double z) {
    ...
    public static void doBaseTick(ReaperPlayer player) {
    ...
    private static void updateFluidOnEyes(ReaperPlayer player) {
    ...
    private static void updateInWaterStateAndDoFluidPushing(ReaperPlayer player) {
    ...
    public static void updatePowderSnow(ReaperPlayer player) {
    ...
    // 1.14
    public static void updatePlayerPose(ReaperPlayer player) {
    ...
    // 1.13 and below
    private static void updatePlayerSize(ReaperPlayer player) {
        Pose pose;
        if (player.isGliding) {
            pose = Pose.FALL_FLYING;
        } else if (player.isInBed) {
    ...
    private static void updateSwimming(ReaperPlayer player) {
    ...
    private static void moveTowardsClosestSpace(ReaperPlayer player, double xPosition, double zPosition) {
    ...
    private static void moveTowardsClosestSpaceLegacy(ReaperPlayer player, double x, double z) {
        int floorX = ReaperMath.floor(x);
        int floorZ = ReaperMath.floor(z);
        int floorY = ReaperMath.floor(player.lastY + 0.5);
    ...
    // 1.14+
    private static void moveTowardsClosestSpaceModern(ReaperPlayer player, double xPosition, double zPosition) {
    ...
    public static void updateInWaterStateAndDoWaterCurrentPushing(ReaperPlayer player) {
        final PacketEntity riding = player.compensatedEntities.self.getRiding();
    ...
    private static boolean updateFluidHeightAndDoFluidPushing(ReaperPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(ReaperPlayer player, FluidTag tag, double multiplier) {
        SimpleCollisionBox aABB = player.boundingBox.copy().expand(0, -0.4, 0).expand(-0.001);

        int floorX = ReaperMath.floor(aABB.minX);
        int ceilX = ReaperMath.ceil(aABB.maxX);
        int floorY = ReaperMath.floor(aABB.minY);
        int ceilY = ReaperMath.ceil(aABB.maxY);
        int floorZ = ReaperMath.floor(aABB.minZ);
        int ceilZ = ReaperMath.ceil(aABB.maxZ);
    ...
    private static boolean updateFluidHeightAndDoFluidPushingModern(ReaperPlayer player, FluidTag tag, double multiplier) {
    ...
    private static boolean suffocatesAt(ReaperPlayer player, int x, int z) {
    ...
    private static boolean clearAbove(ReaperPlayer player, int x, int y, int z) {
