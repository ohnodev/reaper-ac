...
import ac.reaper.predictionengine.PlayerBaseTick;
import ac.reaper.predictionengine.predictions.PredictionEngine;
import ac.reaper.predictionengine.predictions.PredictionEngineElytra;
...
import ac.reaper.utils.nmsutil.EntityTypeTags;
import ac.reaper.utils.nmsutil.FluidFallingAdjustedMovement;
import ac.reaper.utils.nmsutil.GetBoundingBox;
import ac.reaper.utils.nmsutil.MainSupportingBlockPosFinder;
import ac.reaper.utils.team.EntityPredicates;
import ac.reaper.utils.team.EntityTeam;
...
public class MovementTicker {
    public final ReaperPlayer player;

    public static void handleEntityCollisions(ReaperPlayer player) {
    ...
    public void move(Vector3dm inputVel, Vector3dm collide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_18_2)) {
            boolean xAxis = !ReaperMath.equal(inputVel.getX(), collide.getX());
            boolean zAxis = !ReaperMath.equal(inputVel.getZ(), collide.getZ());
        ...
        if (collide.lengthSquared() <= 1e-7
            ...
            player.addMovementThisTick(new ReaperPlayer.Movement(from, to, new Vector3d(inputVel.getX(), inputVel.getY(), inputVel.getZ())));
        }
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(ReaperMath.floor(player.lastX), ReaperMath.floor(player.lastY), ReaperMath.floor(player.lastZ));
        ...
        if (player.wasTouchingWater && !player.isFlying) {
            ...
            player.friction = swimFriction; // Not vanilla, just useful for other reaper stuff
            ...
            if (player.wasTouchingLava && !player.isFlying && !(lavaLevel > 0 && canStandOnLava())) {
                player.friction = 0.5F; // Not vanilla, just useful for other reaper stuff
                ...
                if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_21_5) && Collisions.onClimbable(player, player.lastX, player.lastY, player.lastZ)) {
                    ...
                    player.friction = 0.99F; // Not vanilla, just useful for other reaper stuff
